...
#     t="$SNAP_ROOT/software/tools/stage1                                                -v  " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     4..timeout endless
#e    t="$SNAP_ROOT/software/tools/stage1                                         -t10   -v  " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" # invalid option -t
t="$SNAP_ROOT/software/tools/stage2 -h"                                                  ;echo -e "$del\n$t $l";             $t;echo "RC=$?" #
#     t="$SNAP_ROOT/software/tools/stage2                                         -t10       " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     4..105
#     t="$SNAP_ROOT/software/tools/stage2     -s2 -e4 -i1                         -t10       " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     2..33
#     t="$SNAP_ROOT/software/tools/stage2 -a1                                     -t200"       ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     4..112..33min
#     t="$SNAP_ROOT/software/tools/stage2 -a1 -s2 -e4 -i1                         -t10"        ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     2..34
#     t="$SNAP_ROOT/software/tools/stage2 -a1 -s2 -e8 -i1                         -t100"       ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     5..76..12min
#e    t="$SNAP_ROOT/software/tools/stage2 -a2                                            -vvv" ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" # memcmp failed
#e    t="$SNAP_ROOT/software/tools/stage2 -a2 -z0                                 -t50   -v  " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" # memcmp failed
#     t="$SNAP_ROOT/software/tools/stage2 -a2 -z1                                 -t100      " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     4
#t    t="$SNAP_ROOT/software/tools/stage2 -a2 -z2                                 -t500  -v  " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" # timeout
#     t="$SNAP_ROOT/software/tools/stage2 -a3                                     -t10       " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     1
#     t="$SNAP_ROOT/software/tools/stage2 -a4                                     -t10       " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     1..2
#     t="$SNAP_ROOT/software/tools/stage2 -a5                                     -t10       " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     1..3
#e    t="$SNAP_ROOT/software/tools/stage2 -a6                                            -vvv" ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" # memcmp error
#     t="$SNAP_ROOT/software/tools/stage2 -a6 -z1                                 -t100      " ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     6..10
for align in 4096 1024 256 64; do
for num4k in 0 1; do
for num64 in 1 2; do
t="$SNAP_ROOT/software/tools/stage2 -a2 -A${align} -S${num4k} -B${num64} -t100"        ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #
done
done
done
...
t="$SNAP_ROOT/software/tools/stage2 -a6 -A${align} -S${num4k} -B${num64} -t100"        ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #
done
done
done
...
t="$SNAP_ROOT/software/tools/stage2_ddr -s${strt} -e${end} -b${bsize} -i${iter} -t100" ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #
done
done
done

#### use memset in host or in fpga memory
t="$SNAP_ROOT/software/tools/stage2_set -h"                                              ;echo -e "$del\n$t $l";             $t;echo "RC=$?" #
for beg in 0 1 11 63 64;do         # start adr
for size in 1 7 4097; do           # block size to copy
t="$SNAP_ROOT/software/tools/stage2_set -H -b${beg} -s${size} -p${size} -t100"         ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #
t="$SNAP_ROOT/software/tools/stage2_set -F -b${beg} -s${size} -p${size} -t100"         ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #
done
done
fi

if [[ $action == *"hls_mem"* || $action == *"hls_search"* ]];then echo "testing snap_memcopy"
t="$SNAP_ROOT/software/examples/snap_memcopy -h"                                         ;echo -e "$del\n$t $l";             $t;echo "RC=$?" #     5..7
#     t="$SNAP_ROOT/software/examples/snap_memcopy -C0 -i ../../1KB.txt -o 1KB.out -t10"       ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     5..7
...
# t="$SNAP_ROOT/software/examples/snap_memcopy -i ${size}.in -o ${size}.out -v -t20"   ;echo -e "$del\n$t $l"; # memcopy without checking behind buffer
t="$SNAP_ROOT/software/examples/snap_memcopy -i ${size}.in -o ${size}.out -v -X -t20";echo -e "$del\n$t $l"; # memcopy with checking behind buffer
...
if [[ $action == *"hls_search"* ]];then echo "testing snap_search"
t="$SNAP_ROOT/software/examples/snap_search -h"                                          ;echo -e "$del\n$t $l";             $t;echo "RC=$?" #     7       result=1   ok
#     t="$SNAP_ROOT/software/examples/snap_search -p'A' -C0 -i ../../1KB.txt   -t100      "    ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #    31..34
#     t="$SNAP_ROOT/software/examples/snap_search -pX       -i ../../1KB.txt   -t100      "    ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #    32..35
#     t="$SNAP_ROOT/software/examples/snap_search -p0123    -i ../../1KB.txt   -t500      "    ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #    33
#     t="$SNAP_ROOT/software/examples/snap_search -ph       -i ../../1KB.txt   -t100      "    ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #    31..32
#     t="$SNAP_ROOT/software/examples/snap_search -ph       -i ../../1KB.txt   -t100  -vvv"    ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #    33
#     t="$SNAP_ROOT/software/examples/snap_search -p.       -i ../../fox1.txt  -t30   -v  "    ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #
#     t="$SNAP_ROOT/software/examples/snap_search -p.       -i ../../fox10.txt -t80   -v  "    ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #
#     t="$SNAP_ROOT/software/examples/snap_search -px       -i ../../fox1.txt  -t30   -v  "    ;echo -e "$del\n$t $l";((n++));time $t;echo "RC=$?" #     7       result=1   ok
...
t="$SNAP_ROOT/software/examples/snap_search -p${char} -i${size}.in -E${count} -t$to -v";echo -e "$t $l";((n++));time $t;echo "RC=$?"
done
fi

if [[ $action == *"hls_hash"* ]];then echo "testing snap_hashjoin"
fi
