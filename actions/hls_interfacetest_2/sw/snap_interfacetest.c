...
int main(int argc, char *argv[])
{
    ...
    while (number < 10)
    {
        ...
        while (flag != 1){
        	flag=vol_obuff[1];
                fprintf(stdout, "ibuff=%d, obuff=%d, inbuff_done=%d, outbuff_done=%d, final=%d, var=%d, cnt=%d, in_var=%d\n", vol_ibuff[0], vol_obuff[0], vol_ibuff[1], flag, vol_obuff[2], var, vol_obuff[3], vol_obuff[4]);
