...
int main(int argc, char *argv[])
{
	...
	addr_out = (unsigned long)obuff;
        ...
        while (number < 1000)
        {
        memcpy(ibuff, &var, N*N*sizeof(int32_t));
	...
	snap_prepare_interfacetest(&cjob, &mjob,
			     (void *)addr_in,  sizeof(int32_t), type_in,
			     (void *)addr_out, 0, type_out);
