...
int main(int argc, char *argv[])
{
    ...
    ibuff = snap_malloc(N*N*sizeof(int32_t)); //64Bytes aligned malloc
    if (ibuff == NULL)
    goto out_error;
    memset(ibuff, 0, N*N*sizeof(int32_t));
    ...
    obuff = snap_malloc(N*N*sizeof(int32_t)); //64Bytes aligned malloc
    if (obuff == NULL)
    goto out_error;
    ...
    while (number < 1000)
    {
        memcpy(ibuff, &var1, N*N*sizeof(int32_t));
        ...
        snap_prepare_interfacetest(&cjob, &mjob,
                                   (void *)addr_in,  N*N*sizeof(int32_t), type_in,
                                   (void *)addr_out, N*N*sizeof(int32_t), type_out);
        ...
        memcpy(&result, obuff, N*N*sizeof(int32_t));
        
        printf("post result value is:\n");
        for(ii=0; ii<N; ii++)
        {
            for(jj=0; jj<N; jj++)
            {
        ...
        //printf("var value after CAPI card is %d/n", var);
        for(ii=0; ii<N; ii++)
        {
            for(jj=0; jj<N; jj++)
            {
                result[ii][jj] = result[ii][jj] * 2;
            }
        }
